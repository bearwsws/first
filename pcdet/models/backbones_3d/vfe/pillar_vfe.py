...
class Radar7PillarVFE(VFETemplate):
    def __init__(self, model_cfg, num_point_features, voxel_size, point_cloud_range):
        super().__init__(model_cfg=model_cfg)   # 调用父类VFETemplate的初始化函数

        num_point_features = 0   # 初始化点特征数量为0
        ...
        if self.use_xyz:  # # 如果使用x,y,z坐标，则增加3个通道，并记录对应索引
        ...
        self.pfn_layers = nn.ModuleList(pfn_layers)#将所有PFNLayer封装为ModuleList，便于网络管理
        ...
        ## 计算并保存偏移量，用于确定每个voxel的中心坐标：
        ## offset = voxel_size / 2 + point_cloud_range起始坐标
        self.x_offset = self.voxel_x / 2 + point_cloud_range[0]
        self.y_offset = self.voxel_y / 2 + point_cloud_range[1]
        self.z_offset = self.voxel_z / 2 + point_cloud_range[2]

    def get_output_feature_dim(self):
        return self.num_filters[-1]  # number of outputs in last output channel# 返回最后一层PFNLayer的输出特征维度

    def get_paddings_indicator(self, actual_num, max_num, axis=0):
         # 生成padding指示器，用于标记每个voxel中真实点数与最大点数的关系
        actual_num = torch.unsqueeze(actual_num, axis + 1)
        max_num_shape = [1] * len(actual_num.shape)
        max_num_shape[axis + 1] = -1
        max_num = torch.arange(max_num, dtype=torch.int, device=actual_num.device).view(max_num_shape)
